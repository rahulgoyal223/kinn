...
public class AM_BM2_Medicare_EOE_HH2664 {

	/************************************************************************************
	'Class name                     : 	AM_BM2_Medicare_EOE_HH2664
	'JIRA ID						:	HH-2664
	'Description                    : 	Verify that user is able to Approve an EOE claim
	'Input Parameters           	: 	Patient Name, Claim Number, MRN
	'Output Parameters        		: 	
	'Assumptions                    : 	Test Data is present in the Global Sheet.
	'Use                            : 	The following test verifies that user is able to Approve an EOE claim
	'Tags                           : 	Regression
	 ************************************************************************************/
    ...
    Report.generateReportsFile("html","AM_BM2_Medicare_EOE_HH2664");
    Report.SetTestName("AM_BM2_Medicare_EOE_HH2664", "Verify that user is able to Approve an EOE claim");
    Report.assignCategory("EOE");
    AM.Login.openAppAndSubmitCredentials();
    WebDriver driver = Browser.getDriver();
    String dataFileName = "Claims\\Medicare\\AM_BM2_Medicare_EOE_HH2664.xlsx";
    dataSheetName = "AM_BM2_Medicare_EOE_HH2664";
    ...
    if(Datatable.GetValue("CMS485Check").equals("Yes")){
	   ...
	   AM.Billing.Claims.ClaimsManager.VerifyAlertmessage(driver, "Palmetto GBA: Claim(s) have been generated successfully and can be reviewed in the Pending Payment tab.");
		...
		if(!GlobalData.getPatientMRNumber().isEmpty()){
		 ...
		 Datatable.loadDataSheet(dataFileName, "AM_BM2_Medicare_EOE_HH2664");
		 AM.Menu.TopMenu.Select(driver,"Go To/Billing Manager");
		 ...
		 if(!GlobalData.getPatientMRNumber().isEmpty()){
			...
			//@Step - Verifying the Patient in Ready to Send tab
			Thread.sleep(Waits.getSleepLevelFive());
			AM.Billing.Claims.ClaimsManager.SelectStatusTab(driver, "Ready To Send");
			if(!GlobalData.getPatientMRNumber().isEmpty()){
